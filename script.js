function flyTicket(ticket, isIncrement) {
    ...
    totalAmount();
    completeAmount(ticket, ticketNewCount);
  }
  ...
  function totalAmount() {
    const firstClassBill = getCount('first-class');
    const economyCount = getCount('economy');
    const subtotal = firstClassBill * 150 + economyCount * 100;
  ...
  function fullAmount(ticket, ticketCount) {
    ...
    handleId('Complete-amount').innerText = totalAmount();
  }
...
function displayProperty(display) {
  document
    .getElementsByClassName('Complete-amount-area')[0]
